/*
   Ovládání solárního ohřevu bazénu V8 od petous22
   Solar colector control system V8 wdt by petous22
   Arduino UNO WiFi REV2. 2025
*/
//*******************************************************************************
...
//*******************************************************************************
int reconnectDelay = 1000;            // Počáteční zpoždění v milisekundách
const int maxReconnectDelay = 60000;  // Maximální zpoždění v milisekundách (1 minuta)
...
void StartWiFi() {
  ...
  while (status != WL_CONNECTED) {
    ...
    wdt_reset(); // Důležité pro watchdog!
  }
  reconnectDelay = 1000;
}
...
void setup() {
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Bazen Solar v.7 2025");
  lcd.setCursor(0, 1);
  ...
  //ArduinoOTA.setHostname("WiFiSOLAR"); // Nastavte si vlastní název
  //ArduinoOTA.setPassword("moje_heslo"); // Nastavte si heslo pro zabezpečení (DŮLEŽITÉ!)
